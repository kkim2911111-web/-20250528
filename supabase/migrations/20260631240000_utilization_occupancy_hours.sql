-- 가동률: 해당 월 내 대여 점유 시간(진행 중 포함) — 매출 집계는 변경 없음
...
begin
  ...
  from (
    ...
    left join (
      select
        ...
        coalesce(sum(
          greatest(
            0,
            extract(epoch from (
              least(occ.occ_end, v_period_end)
              - greatest(occ.occ_start, v_period_start)
            )) / 3600.0
          )
      ...
      cross join lateral (
        select
          r.rental_started_at as occ_start,
          case
            when r.status = 'in_use' then now()
            else public.sales_return_completed_at(
              r.returned_at,
              r.actual_end_at,
      ...
      where rv.complex_id = p_complex_id
        ...
        and r.status in ('completed', 'in_use')
        and r.rental_started_at is not null
        and occ.occ_end is not null
        and greatest(occ.occ_start, v_period_start) < least(occ.occ_end, v_period_end)
