...
create or replace function public.get_super_admin_reservation_user_ids()
returns table (
  user_id uuid,
  reservation_id text
)
...
begin
  ...
  return query
  select r.user_id, r.id::text as reservation_id
  from public.reservations r
...
create or replace function public.get_super_admin_reservation_contract(
...
declare
  v_text text;
  v_id text := trim(p_reservation_id);
begin
  perform public.assert_is_super_admin();
  ...
  select nullif(trim(r.contract_content), '')
  into v_text
  from public.reservations r
...
create or replace function public.generate_rental_contract_for_super_admin(
  p_reservation_id text
)
returns jsonb
language plpgsql
...
revoke all on function public.get_super_admin_reservation_user_ids() from public;
grant execute on function public.get_super_admin_reservation_user_ids() to authenticated;

revoke all on function public.get_super_admin_reservation_contract(text) from public;
grant execute on function public.get_super_admin_reservation_contract(text) to authenticated;

revoke all on function public.generate_rental_contract_for_super_admin(text) from public;
grant execute on function public.generate_rental_contract_for_super_admin(text) to authenticated;
