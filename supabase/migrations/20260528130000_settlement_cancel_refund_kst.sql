-- 정산 바텀시트 취소 환불 — Asia/Seoul 월 경계 + payment_orders 취소 포함
...
begin
  ...
  where veh.complex_id = p_complex_id
    ...
    and not exists (
      ...
      where po.reservation_id is not null
        and r2.id::text = po.reservation_id::text
...
comment on function public.get_super_admin_settlement_reservations(uuid, integer, integer) is
  '정산 상세 — total_paid, cancel_refund(reservations·payment_orders 취소, KST 월), items';
