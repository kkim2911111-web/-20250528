...
class AdminLicenseReviewList extends StatefulWidget {
  final AdminService admin;
  final Future<List<LicenseReviewItem>>? future;
  final VoidCallback onReload;

  const AdminLicenseReviewList({
    super.key,
    required this.admin,
  ...
  @override
  State<AdminLicenseReviewList> createState() => _AdminLicenseReviewListState();
}

class _AdminLicenseReviewListState extends State<AdminLicenseReviewList> {
  Future<void> _approve(LicenseReviewItem item) async {
    try {
      await widget.admin.reviewLicense(userId: item.userId, approved: true);
      if (mounted) {
  ...
  Future<void> _reject(LicenseReviewItem item) async {
    ...
    try {
      await widget.admin.reviewLicense(
        userId: item.userId,
        approved: false,
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<LicenseReviewItem>>(
      ...
      builder: (context, snap) {
        ...
        return ListView(
          ...
          children: [
            ...
            ...items.map(
              (item) => AdminLicenseReviewCard(
                item: item,
                onApprove:
                    item.isPendingReview ? () => _approve(item) : null,
                onReject: item.isPendingReview ? () => _reject(item) : null,
              ),
            ),
          ],
        );
...
class AdminLicenseReviewCard extends StatelessWidget {
  final LicenseReviewItem item;
  final VoidCallback? onApprove;
  final VoidCallback? onReject;

  const AdminLicenseReviewCard({
    super.key,
    required this.item,
    this.onApprove,
    this.onReject,
